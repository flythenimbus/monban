			...
			{plugins.length > 0 && (
				<section className="space-y-3">
					...
					{plugins.map((p) => {
						...
						const isUninstalling = pendingUninstall?.name === p.name;
						const isSaving = pendingSave?.name === p.name;
						...
						return (
							<CollapsibleCard
								...
								footer={
									isUninstalling ? (
										<PinAuth
											label={`Authenticate to uninstall ${p.display_name || p.name}`}
											onSubmit={confirmUninstall}
											onCancel={() => setPendingUninstall(null)}
										/>
									) : isSaving ? (
										<PinAuth
											label={`Authenticate to save ${p.display_name || p.name} settings`}
							...
							>
								...
								<div className="pt-2 flex justify-end gap-2">
									<Button
										...
										disabled={pendingUninstall !== null || pendingSave !== null}
									>
										Uninstall
									</Button>
									{hasSettings && (
										<Button
											...
											disabled={
												pendingSave !== null || pendingUninstall !== null
											}
										>
											Save
										</Button>
